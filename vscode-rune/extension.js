...
async function previewRender(input) {
  ...
  if (!panel) {
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
      if (message?.type === "hotload") {
        const current = await vscode.workspace.openTextDocument(sourceUri);
        await compilePreview(current, panel);
      } else if (message?.type === "reload") {
        // Keep the Webview document alive, discard only preview state, then
        // compile again. Calling location.reload() loses the last render message
        // and leaves a blank page because the extension does not resend it.
...
function previewHtml(webview) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; script-src 'nonce-${nonce}' 'unsafe-eval';"><style>:root{color-scheme:light dark}*{box-sizing:border-box}body{margin:0;padding:24px;min-height:100vh;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:var(--vscode-editor-foreground);background:linear-gradient(145deg,var(--vscode-sideBar-background),var(--vscode-editor-background))}.preview-window{overflow:hidden;max-width:1100px;margin:0 auto;border:1px solid color-mix(in srgb,var(--vscode-editor-foreground) 18%,transparent);border-radius:13px;background:var(--vscode-editor-background);box-shadow:0 18px 48px #0005}.titlebar{display:flex;align-items:center;height:46px;padding:0 15px;background:color-mix(in srgb,var(--vscode-titleBar-activeBackground) 70%,var(--vscode-editor-background));border-bottom:1px solid color-mix(in srgb,var(--vscode-editor-foreground) 12%,transparent)}.traffic-lights{display:flex;gap:8px}.light{width:12px;height:12px;padding:0;border:0;border-radius:50%;box-shadow:inset 0 1px #fff6;cursor:default}.light[data-action]{cursor:pointer}.light[data-action]:hover{filter:brightness(1.12);box-shadow:inset 0 1px #fff8,0 0 0 2px #fff4}.close{background:#ff5f57}.minimize{background:#febc2e}.maximize{background:#28c840}.window-title{position:absolute;left:0;right:0;text-align:center;font-size:12px;font-weight:600;letter-spacing:.01em;pointer-events:none;opacity:.82}#status{margin-left:auto;font-size:12px;color:var(--vscode-descriptionForeground)}.content{min-height:300px;padding:28px;background:var(--vscode-editor-background)}#error{margin:0 0 16px;padding:12px;border:1px solid color-mix(in srgb,var(--vscode-errorForeground) 45%,transparent);border-radius:7px;white-space:pre-wrap;color:var(--vscode-errorForeground);background:color-mix(in srgb,var(--vscode-errorForeground) 8%,transparent)}#preview{min-height:220px}</style></head><body><section class="preview-window"><header class="titlebar"><div class="traffic-lights"><i class="light close" aria-hidden="true"></i><button class="light minimize" id="hotload" data-action="hotload" title="Hotload: compile and update while preserving reactive state" aria-label="Hotload preview"></button><button class="light maximize" id="reload" data-action="reload" title="Reload: reset state and render again" aria-label="Reload preview"></button></div><span class="window-title">Rune Preview</span><span id="status">Waiting for Rune preview…</span></header><div class="content"><pre id="error" hidden></pre><main id="preview"></main></div></section><script nonce="${nonce}">const vscode=acquireVsCodeApi(),status=document.querySelector('#status'),error=document.querySelector('#error'),mount=document.querySelector('#preview');document.querySelector('#hotload').addEventListener('click',()=>vscode.postMessage({type:'hotload'}));document.querySelector('#reload').addEventListener('click',()=>vscode.postMessage({type:'reload'}));window.addEventListener('message',({data})=>{if(data.type==='reset'){globalThis.__runePreviewState=new Map();mount.replaceChildren();status.textContent='Resetting preview…';error.hidden=true;return}if(data.type==='loading'){status.textContent='Compiling…';error.hidden=true;return}if(data.type==='error'){status.textContent='Preview failed';error.textContent=data.message;error.hidden=false;return}if(data.type==='render'){try{mount.replaceChildren();new Function('mount',data.source+'\\nrunePreviewSignalIndex = 0;\\nconst __runePreviewResult = typeof render === "function" ? render() : undefined;\\nif (__runePreviewResult instanceof Node) mount.appendChild(__runePreviewResult);')(mount);status.textContent='Preview updated';error.hidden=true}catch(exception){status.textContent='Preview failed';error.textContent=exception.stack||String(exception);error.hidden=false}}});</script></body></html>`;
}
